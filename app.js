...
function updateOcrModeUI(){
  ...
  // Les modes Mistral/Claude/GLM ont besoin du réseau : si on est hors-ligne,
  // on prévient clairement plutôt que de laisser l'utilisateur lancer un scan
  // qui échouera avec une erreur réseau peu compréhensible.
  if(offlineNotice){
...
// Si la connexion tombe ou revient pendant que l'utilisateur est sur l'écran
// de scan, on met à jour l'avertissement immédiatement (pas seulement au
// moment où il change de mode OCR).
window.addEventListener('online', updateOcrModeUI);
window.addEventListener('offline', updateOcrModeUI);
...
Extrait TOUT le texte visible tel qu'il apparaît, ligne par ligne, sans interpréter ni reformuler, en gardant la structure d'origine et en n'omettant aucune date entre parenthèses. Sois particulièrement attentif et précis sur ces 5 informations, qui seront ensuite extraites automatiquement de ton texte : le nom complet de l'enfant, le nom de la mère, le sexe, le lieu de naissance, et la date de naissance. Réponds UNIQUEMENT avec le texte brut extrait, rien d'autre, pas de commentaire, pas de markdown.`;
...
/* ---------- Mistral API OCR ----------
   ...
   comme chez Anthropic). On utilise mistral-medium-latest : pixtral-large-latest
   a été déprécié (27/02/2026) et remplacé par Mistral Medium 3.5, qui a une
   vision entraînée depuis zéro et reste accessible sur le tier gratuit.       */
async function runMistralOcr(imageDataUrl, apiKey){
  ...
  if(!response.ok){
    ...
    if(response.status === 404){
      throw new Error('Modèle Mistral introuvable (probablement déprécié) — vérifie le nom du modèle sur docs.mistral.ai/models/overview.');
    }
    if(response.status === 429){
      throw new Error('Trop de requêtes envoyées — attends quelques secondes et réessaie.');
    }
...
function parseWrittenDate(text){
  ...
  // partie de l'alternance répétée. Le lookahead de fin est volontairement large :
  // certains fournisseurs OCR (ex. Mistral) n'ajoutent pas toujours de ponctuation
  // ou ne recopient pas l'heure après l'année — on accepte donc aussi un simple
  // saut vers un nouveau mot en capitales, ou la fin du texte.
  ...
  const re = new RegExp(
    '\\b((?:[a-zéû]+-?){1,4})\\s+(janvier|f[ée]vrier|mars|avril|mai|juin|juillet|ao[uû]t|septembre|octobre|novembre|d[ée]cembre)\\s+((?:' + yearWord + '[\\s-]*){1,8})(?=\\s*(?:\\bA\\b|\\bÀ\\b|[.,|]|$))',
    'i'
  );
  let m = text.match(re);
  if(!m){
    // Filet de sécurité : si le lookahead strict n'a rien trouvé (ponctuation
    // ou heure absente/inattendue), on retente sans aucune contrainte de fin —
    // on capture juste la plus longue suite de mots-nombres après le mois.
    const reLoose = new RegExp(
      '\\b((?:[a-zéû]+-?){1,4})\\s+(janvier|f[ée]vrier|mars|avril|mai|juin|juillet|ao[uû]t|septembre|octobre|novembre|d[ée]cembre)\\s+((?:' + yearWord + '[\\s-]*){1,8})',
      'i'
    );
    m = text.match(reLoose);
  }
  if(!m) return '';
...
function parseActeFields(rawText){
  ...
  if(sexeLineIdx >= 0){
    for(let i = sexeLineIdx; i >= Math.max(0, sexeLineIdx - 3); i--){
      ...
      const candidate = i === sexeLineIdx ? lines[i].replace(/de sexe.*/i, '') : lines[i];
      const m = candidate.match(/^[\s=.|]*([A-ZÀ-Ü][A-ZÀ-Ü\-']{1,}(?:\s[A-ZÀ-Ü][A-ZÀ-Ü\-']{1,}){1,4})[\s=.|]*$/);
      if(m){ fullNameMatch = m; break; }
    }
  }
  // Filet de sécurité n°1 : certains fournisseurs (ex. Mistral) compressent le
  // document en une seule ligne ou très peu de lignes, ce qui rend l'approche
  // ci-dessus inopérante (sexeLineIdx pointe alors sur un texte fourre-tout).
  // On cherche directement dans flatNoLines le dernier bloc de capitales (2 à 5
  // mots) qui apparaît IMMÉDIATEMENT avant "de sexe", peu importe les séparateurs
  // entre les deux — c'est la position la plus fiable, quelle que soit la mise en page.
  if(!fullNameMatch){
    const m = flatNoLines.match(/([A-ZÀ-Ü][A-ZÀ-Ü\-']{1,}(?:\s[A-ZÀ-Ü][A-ZÀ-Ü\-']{1,}){1,4})\s*[=.,|\s]*\bde\s+sexe\b/i);
    if(m) fullNameMatch = m;
  }
  // Filet de sécurité n°2 : ancienne heuristique basée sur les séparateurs "|" du flat.
  if(!fullNameMatch){
