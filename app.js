...
function parseActeFields(rawText){
  ...
  // Mentions marginales
  // Mentions marginales : le texte peut continuer sur la ligne suivante quand il
  // est trop long pour une seule ligne du formulaire (ex: référence à un acte de
  // notoriété suppletif qui cite un numéro et une date). On autorise donc la
  // capture à franchir UN saut de ligne ("|") tant qu'elle n'a pas encore
  // rencontré de séparateur de fin "===" ou le mot "Certifié" qui suit toujours.
  fields.remarques = grab(/mentions?\s+marginales?\s*[:\-]?\s*([A-Za-zÀ-ÿ0-9°\/\-,.'’ ]{2,80}(?:\s*\|\s*[A-Za-zÀ-ÿ0-9°\/\-,.'’ ]{2,80})?)\s*(?:={2,}|\s*\|\s*Certifi|$)/i)
    .replace(/\s*\|\s*/g, ' ').trim();
