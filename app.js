...
const OCR_PROVIDERS = {
  mistral: {
    ...
    hint: 'Tier gratuit "Experiment" disponible — clé sur <a href="https://console.mistral.ai" target="_blank">console.mistral.ai</a>. Aucune carte bancaire requise pour commencer.'
  },
  deepseek: {
    storageKey: 'deepseek_api_key',
    placeholder: 'Clé API DeepSeek',
    hint: '5 millions de tokens offerts à l\'inscription sans carte bancaire — clé sur <a href="https://platform.deepseek.com" target="_blank">platform.deepseek.com</a>.'
  },
  grok: {
    storageKey: 'grok_api_key',
    placeholder: 'Clé API xAI (Grok)',
    hint: 'Nécessite un compte avec crédit prépayé — clé sur <a href="https://console.x.ai" target="_blank">console.x.ai</a>. Pas de tier gratuit actuellement.'
  },
  claude: {
    storageKey: 'anthropic_api_key',
    placeholder: 'sk-ant-... (clé API Anthropic)',
    hint: 'Facturation à l\'usage, pas de tier gratuit généreux — clé sur <a href="https://console.anthropic.com" target="_blank">console.anthropic.com</a>.'
  },
  glm: {
    storageKey: 'glm_api_key',
    placeholder: 'Clé API Z.ai (GLM)',
    hint: 'Crée un compte sur <a href="https://z.ai" target="_blank">z.ai</a>. Vérifie que ton compte a accès à un modèle GLM-V (vision) avant de l\'utiliser.'
  }
};
...
document.getElementById('runOcrBtn').addEventListener('click', async ()=>{
  ...
  try{
    ...
    if(currentOcrMode === 'tesseract'){
      ...
      // Les fournisseurs IA comprennent l'image et renvoient directement les 5
      // champs déjà structurés en JSON — pas besoin de regex ici.
      const provider = OCR_PROVIDERS[currentOcrMode];
      ...
      } else if(currentOcrMode === 'deepseek'){
        progressLabel.textContent = 'Analyse par DeepSeek en cours…';
        result = await runDeepseekOcr(currentImageDataUrl, apiKey);
      } else if(currentOcrMode === 'grok'){
        progressLabel.textContent = 'Analyse par Grok en cours…';
        result = await runGrokOcr(currentImageDataUrl, apiKey);
...
/* ---------- DeepSeek API OCR ----------
   Format compatible OpenAI, endpoint api.deepseek.com.
   DeepSeek V4 (flash/pro) supporte maintenant la vision (images).
   5M tokens offerts à l'inscription sans carte bancaire.              */
async function runDeepseekOcr(imageDataUrl, apiKey){
  let response;
  try{
  ...
  if(!response.ok){
    const errBody = await response.text();
    if(response.status === 401){
      throw new Error('Clé API DeepSeek invalide ou expirée — vérifie-la sur platform.deepseek.com.');
    }
    if(response.status === 429){
      throw new Error('Trop de requêtes envoyées — attends quelques secondes et réessaie.');
    }
  ...
  const fields = parseOcrJsonResponse(rawText);
  if(!fields){
    throw new Error('DeepSeek n\'a pas renvoyé un JSON valide — réessaie, ou vérifie le texte brut.');
  }
  return { fields, rawText };
}

/* ---------- Grok (xAI) API OCR ----------
   Format compatible OpenAI, endpoint api.x.ai.
   Modèle grok-4.3 (le plus récent au moment de cette version), supporte
   les images en base64 dans les messages. Pas de tier gratuit actuellement —
   nécessite un compte avec crédit prépayé sur console.x.ai.            */
async function runGrokOcr(imageDataUrl, apiKey){
  let response;
  try{
  ...
  if(!response.ok){
    const errBody = await response.text();
    if(response.status === 401){
      throw new Error('Clé API Grok invalide ou expirée — vérifie-la sur console.x.ai.');
    }
    if(response.status === 429){
      throw new Error('Trop de requêtes envoyées — attends quelques secondes et réessaie.');
    }
  ...
  const fields = parseOcrJsonResponse(rawText);
  if(!fields){
    throw new Error('Grok n\'a pas renvoyé un JSON valide — réessaie, ou vérifie le texte brut.');
  }
  return { fields, rawText };
}
